...
-- -- CAU 11
-- GO
-- CREATE TRIGGER HOADON_TRIGGER ON HOADON FOR INSERT,UPDATE AS
-- BEGIN
--     IF EXISTS (
--         SELECT *
--         FROM inserted I,KHACHHANG KH
--         WHERE I.MAKH = KH.MAKH AND I.NGHD < KH.NGDK )
--         BEGIN
--             RAISERROR('GIA TRI KHONG HOP LE',0,1);
...
--         WHERE I.MAKH = HD.MAKH AND I.NGDK > HD.NGHD )
--         BEGIN
--             RAISERROR('GIA TRI KHONG HOP LE',0,1);
--             ROLLBACK TRANSACTION
--         END 
...
-- INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (1024,'14/11/2006','KH79','NV03',250000)
-- INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (1025,'14/11/2006','KH79','NV03',250000)

-- INSERT INTO KHACHHANG(MAKH,HOTEN,DCHI,SODT,NGSINH,NGDK,DOANHSO) VALUES ('KH79','LE HA VINH','837 LE HONG PHONG,Q5,TPHCM','08654763','03/03/1979','14/11/2006',70000)
...
-- -- CAU12
-- GO
-- CREATE TRIGGER NHANVIEN_TRIGGER ON HOADON FOR INSERT,UPDATE AS
-- BEGIN 
--     IF EXISTS (
--         SELECT *
--         FROM inserted I,NHANVIEN NV
--         WHERE I.MANV = NV.MANV AND I.NGHD < NV.NGVL )
--         BEGIN
--             RAISERROR('GIA TRI NGAY KHONG HOP LE',0,1);
--             ROLLBACK TRANSACTION
--         END 
--         ELSE 
--         BEGIN
...
-- GO
-- CREATE TRIGGER HOADONNV_TRIGGER ON NHANVIEN FOR UPDATE AS
-- BEGIN 
--     IF EXISTS(
--         SELECT *
--         FROM inserted I,HOADON HD 
--         WHERE I.MANV = HD.MANV AND I.NGVL > HD.NGHD )
--         BEGIN
--             RAISERROR('GIA TRI NGAY KHONG HOP LE',0,1);
--             ROLLBACK TRANSACTION
--             -- rollback lap trinh giao tac
--         END 
--         ELSE
--         BEGIN 
--             PRINT('THEM THANH CONG ');
...
-- INSERT INTO NHANVIEN(MANV,HOTEN,SODT,NGVL) VALUES ('NV99','NGUYEN NHU NHUT','0927345608','15/04/2006')

-- INSERT INTO HOADON(SOHD,NGHD,MAKH,MANV,TRIGIA) VALUES (2000,'13/04/2006','KH79','NV99',180000)

-- GO
-- CREATE TRIGGER HOADON_TRIGGER_VERSION2 ON HOADON FOR INSERT,UPDATE AS
-- BEGIN
...
--     IF(@NGHD<@NGDK)
--     BEGIN
--         PRINT('ERROR,LOI KHI THEM DU LIEU');
--         ROLLBACK TRANSACTION
--     END
--     ELSE 
--     BEGIN
...
CREATE TABLE CHITIET_PT (
    MAPT CHAR(5),
    MASACH CHAR(5)
    CONSTRAINT CHITIET_MAPT_MASACH_PK PRIMARY KEY(MAPT,MASACH)
)

ALTER TABLE CHITIET_PT ADD 
    CONSTRAINT CHITIET_PT_MAPT_PK FOREIGN KEY (MAPT) REFERENCES PHIEUTHUE(MAPT),
    CONSTRAINT CHITIET_PT_MASACH_PK FOREIGN KEY (MASACH) REFERENCES SACH(MASACH)
...
BEGIN
    IF EXISTS(
        SELECT * 
        FROM inserted PT     
        WHERE YEAR(PT.NGAYTHUE) = YEAR(PT.NGAYTRA) AND MONTH(PT.NGAYTHUE) = MONTH(PT.NGAYTRA) AND (DAY(PT.NGAYTRA) -DAY(PT.NGAYTHUE) >10)
    )
    BEGIN
        RAISERROR('NGAY KHONG HOP LE',0,1);
        ROLLBACK TRANSACTION 
    END
    ELSE 
    BEGIN 
...
-- 2.2 
GO 
CREATE TRIGGER TRIGGER_PHIEUTHUE ON PHIEUTHUE FOR UPDATE AS 
BEGIN

    IF EXISTS(
        SELECT *    
        FROM INSERTED PT,CHITIET_PT CTPT 
        WHERE PT.MAPT = CTPT.MAPT AND PT.SOSACHTHUE != COUNT(CTPT.MAPT)
            )
    BEGIN
        UPDATE PHIEUTHUE    
        SET PHIEUTHUE.SOSACHTHUE = COUNT (CTPT.MAPT)
